...
class FXOTestCase(TestCase):
    ...
    def setUp(self):
        ...
        r = RateQuote.objects.create(ref_date=d, name='USD LIBOR 6M', rate=0.0021, tenor='6M', instrument='DEPO', ccy=usd, day_counter='Actual360')
        rTS = IRTermStructure.objects.create(name='USD LIBOR', ref_date=d, as_fx_curve=usd)
        rTS.rates.add(r)
        q = RateQuote.objects.create(ref_date=d, name='EUR FX 3M', rate=-0.002, tenor='3M', instrument='DEPO', ccy=usd, day_counter='Actual365Fixed')
