...
class FXOTestCase(TestCase):
    ...
    def setUp(self):
        ...
        spot = FxSpotRateQuote.objects.create(ref_date=d,
                                              rate=1.098,
                                              ccy_pair=eurusd)
        vol = FXVolatility.objects.create(ref_date=d,
                                          vol=0.08,
                                          ccy_pair=eurusd)
        ...
        rTS = IRTermStructure.objects.create(name='USD LIBOR',
                                             ref_date=d,
                                             as_fx_curve=usd)
        rTS.rates.add(r)
        ...
        qTS = IRTermStructure.objects.create(name='EUR FX',
                                             ref_date=d,
                                             as_fx_curve=eur)
        qTS.rates.add(r)
